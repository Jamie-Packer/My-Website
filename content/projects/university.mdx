...
This isn't all the project work I did at university, nor is it a complete write-up of each one, just an overview of the relevant project-type work.
...
Given the extra setup and higher credit spend, I favoured the pre-trained baseline for **price–performance**, and would only deploy to a Vertex AI endpoint when a managed, real-time API is required.
...
This coursework compared **Variational Approximation (VA)** and **Markov Chain Monte Carlo (MCMC)** for **Bayesian Neural Networks (BNNs)** on a flood hazard dataset of ~67k samples, where only ~7% were labelled hazardous.
