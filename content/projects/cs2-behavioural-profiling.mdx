...
Before building any models, I needed to answer a fundamental question: *how many maps does a player need before their playstyle metrics stabilise?* 
...
Four CT-side Rotators (**biguzera, chopper, bLitz, and apEX**) were confidently misclassified as AWPers. What do they have in common? They're all **In-Game Leaders (IGLs)**.

Digging into their feature values revealed the cause: all four had unusually low ADAT Residuals, meaning they positioned closer to the team centre than typical Rotators. This "central positioning" pattern mimics the spacing profile of CT AWPers.

Why would IGLs play centrally? **Information flow.** Central positions (think: Connector on Mirage, Middle on Ancient) allow the IGL to process information from both bombsites without relying on callouts. The trade-off is that these are high-engagement areas typically reserved for star players (often AWPers), which explains why IGLs are often criticised for underwhelming stats.

This isn't a novel discovery. Analyst Harry Richards documented the ["supportive rotator" IGL trend in 2024](https://www.hltv.org/news/38747/why-more-and-more-igls-are-taking-up-the-supportive-rotator-role). But seeing the model independently surface this pattern validates both the features and the classification approach.

### Diagnostic Value

Perhaps the most practical outcome: **high-confidence misclassifications often pointed to labelling errors in the ground truth**.
