...
**Potential Utility:** This work demonstrates a clear proof-of-concept for **automated role labelling**, applicable when manual annotation is unfeasible, such as on commercial analytics platforms or for large datasets.
...
*   **Cluster Analysis:** Unsupervised clustering (no role labels used in fitting) recovered groups that align with expert roles, with clearer structure on T-side than CT-side. This validates that these "roles" have real statistical backing in the data, even with just **6 behavioural features**.
...
### Source & Scale
The dataset for this project was constructed via a local ETL process (Python) using raw match replay files (.dem) from the 2024 competitive season.
...
Whilst this would have considerably improved classification scores, out of research interest I wanted to see if the model could identify an AWPer purely by *how they behave*, not just by the gun they hold.
...
<table>
  ...
  <tbody>
    <tr>
      <td rowspan="2"><strong>Aggression</strong></td>
      <td><code>TAPD</code> (Time Alive Per Death)</td>
      <td>Do they die early or survive late?</td>
    </tr>
    <tr>
      <td><code>OAP</code> (Opening Attempt %)</td>
      <td>How often do they take the first duel of the round?</td>
    </tr>
    <tr>
      <td rowspan="2"><strong>Teamwork (Trading)</strong></td>
      <td><code>PODT</code> (Proportion of Deaths Traded)</td>
      <td>When they die, is a teammate close enough to avenge them?</td>
    </tr>
    <tr>
      <td><code>POKT</code> (Proportion of Kills which were Trades)</td>
      <td>Are they the ones doing the avenging?</td>
    </tr>
    <tr>
      <td rowspan="2"><strong>Positioning (Novel)</strong></td>
      <td><code>ADNT</code> (Avg Distance to Nearest Teammate)</td>
      <td>Are they a &quot;pack&quot; player or a lone wolf?</td>
    </tr>
    <tr>
      <td><code>ADAT</code> (Avg Distance to Avg Teammate)</td>
      <td>Do they play central or peripheral angles?</td>
    </tr>
  </tbody>
</table>
...
To solve this, the positional metrics (ADNT/ADAT) are **team-relative**. For every sampled tick, players were ranked 1–5 based on their distance from teammates, then normalised to **(closest) 0.2 {"<->"} 1.0 (farthest)**.
...
<ProjectLinks
  liveUrl="/notebooks/01_eda.html"
  liveLabel="Interactive Report"
  repoUrl="https://github.com/Jamie-Packer/cs2-playstyle-analysis-2024-public/blob/main/notebooks/01_eda.ipynb"
  repoLabel="Notebook"
/>

Before building any models, I needed to answer a fundamental question: 
> *How many matches (maps) does a player need before their playstyle metrics stabilise?* 
...
<PlotlyEmbed
  src="/images/projects/cs2-behavioural-profiling/pos_features_scatter.html"
  title="3D PCA of T-side playstyles"
  caption="Interactive scatter plot of players by ADNT and ADAT, with roles colour labelled"

/>

This correlation pointed to an interesting question: 

> Do some roles position further from the centre than their isolation alone would predict?
...
<ProjectLinks
  liveUrl="/notebooks/02_classification.html"
  liveLabel="Interactive Report"
  repoUrl="https://github.com/Jamie-Packer/cs2-playstyle-analysis-2024-public/blob/main/notebooks/02_classification.ipynb"
  repoLabel="Notebook"
/>
...
And looking at *how* can tell us even more about how pros play!
...
The most interesting finding came from analysing *misclassifications*.
...
<ProjectLinks
  liveUrl="/notebooks/03_clustering.html"
  liveLabel="Interactive Report"
  repoUrl="https://github.com/Jamie-Packer/cs2-playstyle-analysis-2024-public/blob/main/notebooks/03_clustering.ipynb"
  repoLabel="Notebook"
/>

> If you throw the labels away, do playstyles still **naturally form groups**, and do those groups look like the roles analysts already talk about?

**Mostly, yes!**

<PlotlyEmbed
  src="/images/projects/cs2-behavioural-profiling/pca3d_t.html"
  title="3D PCA of T-side playstyles"
  caption="Interactive 3D PCA (T-side). Use the buttons to colour by Role, Ward cluster, or GMM cluster."
  height={720}
/>

I used the same Orthogonal feature set as before (behavioural stats plus the isolation and centrality residuals), ran T and CT separately, and dropped the hybrid labels (Half-Lurker on T, Mixed on CT) so the geometry reflects the core archetypes. Two methods: Hierarchical (Ward) for hard partitions, and a **Gaussian Mixture Model (GMM)** for soft assignments. I fixed **k = 3** (one cluster per core role per side) after checking silhouette scores across k = 2 to 6 to check if it was sensible.

### Soft groups

Silhouette at k = 3 was about **0.17-0.18 on T** and **0.14-0.15 on CT**. So there is structure, but the groups overlap. Spacetakers aren't miles from everyone else, and AWPers share some behavioural DNA with other roles. That is roughly how CS works in practice: roles bleed at the edges.

BIC preferred **tied** covariance for the T-side GMM (three archetypes in different regions of a similarly shaped cloud) and **spherical** on CT (tighter, less elongated defensive structure).

### Agreement with expert roles

<FigureImage
  src="/images/projects/cs2-behavioural-profiling/dendrogram_t.svg"
  alt="Ward dendrogram of T-side playstyles"
  caption="Ward dendrogram of T-side players. Leaf colours are expert roles; cutting at k=3 the clusters largely maps onto each role Spacetaker, Lurker, and AWPer."
  width={1400}
  height={800}
/>

Using **Adjusted Rand Index** (chance-corrected; random partitions score near 0):

*   **T-side:** **0.68 (Ward)** / **0.61 (GMM)**
*   **CT-side:** **0.31 (Ward)** / **0.30 (GMM)**

T-side is roughly twice as strong. Ward largely pulls out Spacetaker, Lurker, and AWPer blocks (the dendrogram above is the T-side view of that). On CT, Anchors separate more cleanly, while Rotators and AWPers mix. Same porosity the classifier hit, this time without ever seeing the labels during fitting.
